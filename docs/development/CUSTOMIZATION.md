...
Every Monday at 07:00 UTC, the workflow checks whether the upstream blueprint (`shyndman/ha-mqtt-media-bridge`) has new commits. If it does, it opens a pull request with the diff against your repository.
...
That's it. No workflow runs, no PRs, no noise. You can still pull upstream changes manually at any time by comparing your repository against `shyndman/ha-mqtt-media-bridge`.
