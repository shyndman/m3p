...
This document describes all configuration options and settings available in the MQTT Media Bridge custom integration.
...
### `mqtt_media_bridge.example_service`
...
```yaml
service: mqtt_media_bridge.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: mqtt_media_bridge.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/shyndman/ha-mqtt-media-bridge/issues) - Report problems
