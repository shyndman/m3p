"""Run a standalone MQTT media player for end-to-end bridge testing."""
...
class Config:
    """Runtime configuration for the mock MQTT media player."""
...
@dataclass
class Track:
    """Generated media metadata and artwork for one simulated track."""
...
class WebSocketMediaPlayer(MediaPlayer):
    ...
    def __init__(
        ...
        """Initialize the player and subscribe to command topics after connecting."""
        LOGGER.debug(
...
class MockPlayer:
    """Simulate a continuously playing device over MQTT discovery."""

    def __init__(self, config: Config) -> None:
        """Initialize player state, publish discovery, and start playback."""
        self._config = config
    ...
    def stop(self) -> None:
        """Stop playback and remove the player from MQTT discovery."""
...
def main() -> None:
    """Run the mock player until an interrupt or termination signal."""
