## 1. Implementation
- [ ] 1.1 Create `tools/mock-player/` as a standalone `uv` app (`uv init --application`).
- [ ] 1.2 Add dependency on `shyndman/ha-mqtt-discoverable` from GitHub `main` and `Pillow` for PNG generation.
- [ ] 1.3 Add `tools/.env.sample` with all supported settings and defaults; do not commit `tools/.env`.
- [ ] 1.4 Implement `tools/mock-player/src/mock_player/main.py` with:
...
- [ ] 1.5 Add `scripts/mock-player` runner that executes the tool (loading `tools/.env`).
- [ ] 1.6 Add a brief tool README in `tools/mock-player/README.md` with usage and env var reference.

## 2. Validation
- [ ] 2.1 Run `openspec validate mock-media-playback --strict` and fix any issues.
